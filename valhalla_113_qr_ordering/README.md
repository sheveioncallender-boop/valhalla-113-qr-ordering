...
- Includes a branded portrait landing image so a fresh phone opens Odoo’s native self-order landing page.
...
Version `19.0.1.0.2` repairs configurations made with the first release. After upgrading, refresh the QR link on the phone. If the old page is cached, open the **Valhalla QR Ordering** tab and click **Repair Mobile Landing Screen**, then reload Safari. The table QR URL itself does not need to change.


## CSS bundle repair (19.0.1.0.2)

This release uses plain CSS in `pos_self_order.assets`. Delete or replace the complete previous module folder, deploy this version, upgrade the module, restart Odoo, and then test the QR link in a private browser tab. The table QR codes do not need to be reprinted.
